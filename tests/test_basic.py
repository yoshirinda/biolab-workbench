...
class TestRoutes:
    ...
    def test_removed_sequence_page(self, client):
        """Sequence-management UI is intentionally removed from the product scope."""
    ...
    def test_docs_removed(self, client):
        """Separate docs pages are removed from the five-workflow product scope."""
        response = client.get('/docs/')
        assert response.status_code == 404

    def test_workflow_pages_show_scientific_default_badges(self, client):
        response = client.get('/blast/')
        assert response.status_code == 200
        assert b'Standard default' in response.data
    ...
    def test_index_module_links_use_url_for(self, client):
        ...
        assert 'href="/sequence/"' not in html
...
class TestAuth:
    def test_auth_redirects_when_enabled(self):
        app = create_app()
        app.config['TESTING'] = True
        app.config['AUTH_ENABLED'] = True
        app.config['PASSWORD_HASH'] = 'invalid-hash'
        with app.test_client() as client:
            response = client.get('/')
...
class TestRemovedSequenceSurface:
    """Sequence-management APIs are intentionally absent after the product reset."""

    def test_sequence_api_removed(self, client):
        response = client.post('/sequence/import', data={'source': 'text', 'text': '>seq1\nATGC'})
        assert response.status_code == 404

    def test_project_api_removed(self, client):
